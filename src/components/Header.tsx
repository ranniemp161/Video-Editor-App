...
export const Header: React.FC<HeaderProps> = ({
  ...
  const [isWarningDismissed, setIsWarningDismissed] = React.useState(() => {
    // Check if user dismissed the warning in this session
    return sessionStorage.getItem('warningDismissed') === 'true';
  });

  // Warn user before closing tab if project is active (always active, even if banner dismissed)
  React.useEffect(() => {
  ...
  return (
    <>
      {/* Session Warning Banner - Dismissible */}
      {hasActiveProject && !isWarningDismissed && (
        <div className="bg-yellow-500/10 border-b border-yellow-500/20 px-4 py-2 flex items-center justify-between sticky top-0 z-50">
          ...
          <button
            ...
            className="ml-4 p-1 hover:bg-yellow-500/20 rounded transition-colors flex-shrink-0"
            title="Dismiss warning (you'll still be warned when closing the tab)"
          >
